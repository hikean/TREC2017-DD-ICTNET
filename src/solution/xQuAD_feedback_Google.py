...
from src.utils.Google_scorer import *
    ...
    for tid, topic in topics:
            ...
            for i in range(tot_itr_times):
                ...
                if i == 0 or ( not if_use_jig_feedback and len(subqueries) == 0 ):
                    docs_list = irsys_doclist_add_google_score_and_rerank(docs_list, google_scorer, tid)
                    ...
                    while len(this_itr_select_docs) < 5  and file_ptr < len(docs_list):
                        ...
                        #ADD GOOGLE SCORE
                        # gscore = google_scorer.get_score_by_tid_key(tid, docs_list[file_ptr][0])
                        # if gscore is not None:
                        #     print "CHECK USING GSCORE:", gscore
                        #     this_itr_select_docs[-1][1][0] += gscore
                    ...
                    for ixquad_selected in range(every_itr_doc_cnt):
                        ...
                        ranked_docs = xquad_doclist_add_google_score_and_rerank(ranked_docs, google_scorer, tid)
                        ...
                        # gscore = google_scorer.get_score_by_tid_key(tid,  d[0][KEY])
                        print "-----CHECK SCORE SELECTED, [ xquad score + google score, rel_score, div_score google score格式]->>>:", d[1:], d[0]
                        # if gscore is not None:
                        #     print "CHECK GOOGLE SCORE:", gscore
                        #     this_itr_select_docs[-1][1] += gscore
                        # D.append(d[0])
                        # D[-1][SCORE] = d[1]
...
def xQuAD_multisource_with_feedback_google_score(topics = EBOLA_TOPICS,
                suggestor=None,
                if_use_clean_text=True,
                boost_params=1,
                if_stem=True,
    ...
    for tid, topic in topics:
            ...
            for i in range(tot_itr_times):
                ...
                if i == 0 or ( not if_use_jig_feedback and len(subqueries) == 0 ):
                    #TODO:CHECK 看这里的google score处理是不是合适，因为每一轮都是要看
                    docs_list = irsys_doclist_add_google_score_and_rerank(docs_list, google_scorer, tid)
                    file_ptr = 0 #TODO:检查所有的这个变量，，，
                    if len(subqueries) == 0:
                        print "======@@@@@@@@@@@@> subquery cnt is zero, tid, topic:", tid, topic
                    ...
                    if iresult is not None:
                        ...
                        for ir, _ in enumerate(iresult):
                            ...
                            if int(_['on_topic']) == 1:
                                ...
                                not_on_topic_doc_set[this_itr_select_docs[ir][0]] = this_itr_select_docs[ir][1][2]
                                this_itr_not_on_topic_cnt += 1
                        if this_itr_not_on_topic_cnt >= 5: this_topic_use_google = True
                    update_state(already_cover_topic_dict, iresult)
                    continue
                else:
                    ...
                    for ixquad_selected in range(every_itr_doc_cnt):
                        ...
                        if ranked_docs[0][1] == 0:
                            logging.info("FUCK")
                            print "ERROR FUCK HERE:",ranked_docs[0]
                        ranked_docs = xquad_doclist_add_google_score_and_rerank(ranked_docs, google_scorer, tid)
                        while ranked_docs[ptr_][0][KEY] in already_select_key_set:
                            ptr_ += 1
                            continue
                        ...
                        if d[0][KEY] in already_select_key_set:
                            print "############!!!!!!!!!ERROR >>>>>>>>>>>> SELECT DUP:", d[KEY]
                        # if i == 0:
                        print "-----CHECK SCORE SELECTED, [ xquad score, rel_score, div_score google_score格式]->>>:", d[1:], d[0]
                        this_itr_select_docs.append(d)
                        # D.append(d[0])
                        # D[-1][SCORE] = d[1]
                        already_select_key_set.add(d[0][KEY])
                        ...
                        if type(iresult) is list:
                            ...
                            for ir,_ in enumerate(iresult):
                                ...
                                if int(_['on_topic']) == 1:
                                    ...
                                    not_on_topic_doc_set[this_itr_select_docs[ir][0][KEY]] = this_itr_select_docs[ir][0]
                                    this_itr_not_on_topic_cnt += 1
                            if this_itr_not_on_topic_cnt >= 5:
                                this_topic_use_google = True
