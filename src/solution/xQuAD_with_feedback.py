...
def xQuAD_with_feedback_del_dup_dynamic_D(topics = EBOLA_TOPICS,
                suggestor=None,
...
'''
修复几个bug:
1、for i in xxx的i和i itr那里的问题
2、如果是用搜索引擎的结果，那么D放东西是不一样的
'''

def xQuAD_clean_use_local_data_with_feedback_clean_code_confirm_D_fix_bug(topics = EBOLA_TOPICS,
                suggestor=None,
                if_use_clean_text=True,
                boost_params=1,
                if_stem=True,
                    ...
                    if iresult is not None:
                        for ir, _ in enumerate(iresult):
                            ...
                            if _['on_topic'] == 1:
                                ...
                                print "CCC this_itr_select_docs[ir]:", this_itr_select_docs[ir]
                                print "this_itr_select_docs[ir][0]:", this_itr_select_docs[ir][0]
                                not_on_topic_doc_set[this_itr_select_docs[ir][0]] = this_itr_select_docs[ir][1]
                    update_state(already_cover_topic_dict, iresult)
