...
选择当前每个子话题最top的passage_text作为aspect，
'''
def passage_text_to_expand_query(already_cover_topic_dict, user_top_k_text=2, ignore_limit_cnt=4, qe_score_limit=3, minus_limit=2,if_use_minus=False):
    '''
    ...
    for subtopic_id, info in already_cover_topic_dict.items():
        ...
        for rate in RATES:
            # #TODO:这里改成负权重
            # if rate < qe_score_limit:
            #     # if if_use_minus:
            #
            #     continue
            if rate >= qe_score_limit and select_cnt < user_top_k_text and info.has_key(rate):
                one_aspect += info[rate][1]
                if CHECK:
                    print "CHECK passage_text_to_subqueries, rate, info[rate]:", rate, info[rate]
                    CHECK = False
                ...
                print  "subtopic_id", subtopic_id, " select rate:", rate
                print "passage_text_to_subqueries, subtopic_id, use rate:", subtopic_id, rate
            if select_cnt >= user_top_k_text and info.has_key(rate):
                #TODO:这里没有Break的时候能act第二轮0.2...
                if rate <= minus_limit:
                    # if if_use_minus:
                    minus_aspect += info[rate][1]
                    continue

        minus_aspect = ' '.join(minus_aspect)
        one_aspect = ' '.join(one_aspect)
        aspect_sents.append(one_aspect)
        minus_aspect_sents.append( minus_aspect )

    if if_use_minus:
        return aspect_sents, minus_aspect_sents
    else:
        return aspect_sents
...
def dicts2query(q, eq, sp=' ', if_set_true=False):
    '''
...
def dicts2query_rocc(q, eq, neq, a=0.75, b=0.25, sp=' ', set_True=False):
    ret = ''
    ret_word_list = []
    for w in q:
        ret += w + '^' + str(1.0) + sp
    ...
    for w in neq.keys():
        if set_True:
            ...
            ret += w + '^' + str( (-1.0) * b * neq[w]) + sp

    ret = ret.strip()

    return ret, ret_word_list
