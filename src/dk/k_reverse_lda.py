        ...
        for tt in range( int(sys.argv[4]) ):
            ...
            for i in range(5):
                ...
                for file_id in list_doc_q:
                    ...
                    k = max( 10 - len(list_doc_s) , 1 )
                    for topic_id in range(n_topic):
                        key_topic_id = str(topic_id)
                        file_value += k * dict_topic_has[key_topic_id] *  (dict_fileid_topic[file_id][key_topic_id] if key_topic_id in dict_fileid_topic[file_id] else 0.0)
