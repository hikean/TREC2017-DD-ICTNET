...
def get_lda(out_dir,topic_name):
    ...
    file_ldaOut = codecs.open(out_dir + "{}-{}-{}-{}-{}-{}-{}-{}-lda_topics.txt".format(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5],sys.argv[6],sys.argv[7],topic_name), "w", "utf-8")
    file_ldaOut.write(str(ldaOut))
    
    file_id_lda = codecs.open(out_dir + "{}-{}-{}-{}-{}-{}-{}-{}-fileid_lda.txt".format(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5],sys.argv[6],sys.argv[7],topic_name), "w", "utf-8")
...
if __name__ == '__main__':
    ...
    for ebola_query in list_query[0:int(sys.argv[6])]:
        ...
        in_dir_fileid_lda = "data/lda/{}-{}-{}-{}-{}-{}-{}-{}-fileid_lda.txt".format(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5],sys.argv[6],sys.argv[7],topic_name)
        ...
        for tt in range( int(sys.argv[4]) ):
            ...
            float_topic_sum = 0.0
            dict_topic_has = {str(key) : 0.0 for key in range(n_topic)}
            for doc_id in list_doc_s:
                for topic_id in dict_fileid_topic[doc_id]:
                    dict_topic_has[topic_id] += dict_fileid_topic[doc_id][topic_id]
                    float_topic_sum += dict_fileid_topic[doc_id][topic_id]
            #print("dict_topic_has: ",dict_topic_has)
            for i in range(5):
                ...
                for file_id in list_doc_q:
                    ...
                    if float_topic_sum != 0.0:
                        for topic_id in range(n_topic):
                            key_topic_id = str(topic_id)
                            file_value += dict_topic_has[key_topic_id] / float_topic_sum *  (dict_fileid_topic[file_id][key_topic_id] if key_topic_id in dict_fileid_topic[file_id] else 0.0)
