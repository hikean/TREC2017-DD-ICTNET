...
def interact_with_jig(jig, docs, in_dir , interact_times , dict_subtopic_cnt , list_doc_untopic):
    list_re_doc_id = []
    list_unre_doc_id = []
    dict_keyword_score = {}
    st_ptr = 0
    ...
    for i in range(interact_times):
        ...
        for _ in rslt:
            ...
            for dict_subtopics in _["subtopics"]:
                ...
                if subtopic_id not in dict_subtopic:
                    dict_subtopic[subtopic_id] = {}
                    
                dict_rating_text = dict_subtopic[subtopic_id]
                int_rating = dict_subtopics["rating"]
                
                if int_rating not in dict_rating_text:
                    dict_rating_text[int_rating] = []
                
                if dict_subtopics["passage_text"] not in dict_rating_text[int_rating]:
                    dict_rating_text[int_rating].append(dict_subtopics["passage_text"])
                
                dict_subtopic[subtopic_id] = dict_rating_text
    ...
    return list_re_doc_id , list_unre_doc_id , dict_keyword_score , dict_subtopic_cnt , list_doc_untopic
...
def cal_dict_s_topic_pm():
    global dict_subtopic
    
    dict_s_topic = {}
    
    sum = 0.0
    for subtopic in dict_subtopic:
        
        for rating in dict_subtopic[subtopic]:
            score = rating * float( len( dict_subtopic[subtopic][rating] ) )
            sum += score
            if subtopic not in dict_s_topic:
                dict_s_topic[subtopic] = 0.0
            dict_s_topic[subtopic] += score

    for subtopic in dict_s_topic:
        dict_s_topic[ subtopic ] = float( dict_s_topic[ subtopic ] ) / sum
        
    return dict_s_topic

def get_dict_tfidf(list_word):
    dict_tfidf = {}
    for word in list_word:
        if word not in idf_dictionary:
            list_not_in_dictionary.add(word)
            continue
        if word not in dict_tfidf:
            dict_tfidf[word] = 0.0
        dict_tfidf[word] += 1.0
    
    for word in dict_tfidf:
        dict_tfidf[word] = dict_tfidf[word] * idf_dictionary[word]
    
    return dict_tfidf
    
def get_cos(subtopic , fileid , in_dir):
    list_score = [4,3,2,1]
    int_cnt = 0
    str_query = ""
    for int_score in list_score:
        if int_score not in dict_subtopic[subtopic]:
            continue
        for str_text in dict_subtopic[subtopic][int_score]:
            str_query += str_text
            int_cnt += 1
            if int_cnt == 2:
                break
    ...
    js = json.load(codecs.open(in_dir.format(fileid), "r") )
    line = js["content_full_text"]
    seg_list = basic_preprocess(line,"utf8",True,True)
    dict_file_tfidf = get_dict_tfidf(seg_list)
    
    seg_list = basic_preprocess(str_query,"utf8",True,True)
    dict_subtopic_tfidf = get_dict_tfidf(seg_list)
    
    float_mul = 0.0
    for word in dict_file_tfidf:
        if word in dict_subtopic_tfidf:
            float_mul += dict_file_tfidf[word] * dict_subtopic_tfidf[word]
    
    float_mod_file = 0.0
    for word in dict_file_tfidf:
        float_mod_file += dict_file_tfidf[word] * dict_file_tfidf[word]
    float_mod_file = math.sqrt(float_mod_file)
    
    float_mod_subtopic = 0.0
    for word in dict_subtopic_tfidf:
        float_mod_subtopic += dict_subtopic_tfidf[word] * dict_subtopic_tfidf[word]
    float_mod_subtopic = math.sqrt(float_mod_subtopic)
    
    if (float_mod_file == 0.0) or (float_mod_subtopic == 0.0) :
        return 0.0
    else:
        return float_mul / float_mod_file / float_mod_subtopic

if __name__ == '__main__':
    
    global list_doc_q,list_doc_s,lama,lamb,corp,get_id,lsh,lamc
    """
    ...
    jig = JigClient("",tot_itr_times=tot_itr_times,base_jig_dir="/home/zhangwm/Software/dk_jig10/jig/trec-dd-jig/")
    ...
    for ebola_query in NYT_TOPICS[0:int(sys.argv[6])]:
        ...
        for tt in range( int(sys.argv[4]) ):
            ...
            #pm_topic
            dict_s_topic_pm = cal_dict_s_topic_pm()
            
            pm_max_topic_value = -10.0
            pm_max_topic_id = "0"
            pm_quotient = {}
            
            for subtopic in dict_subtopic:
                pm_quotient[subtopic] = 1 / (1.0 + 2 * dict_s_topic_pm[subtopic])
            
                if pm_quotient[subtopic] > pm_max_topic_value :
                    pm_max_topic_value = pm_quotient[subtopic]
                    pm_max_topic_id = subtopic
            ...
            for file_id in list_doc_q:
                ...
                pm_score = 0.0
                for subtopic in dict_subtopic:
                    if subtopic == pm_max_topic_id:
                        pm_score += lamb * pm_quotient[subtopic] *  get_cos(subtopic , file_id , in_dir)
                    else:
                        pm_score += (1.0 - lamb) * pm_quotient[subtopic] *  get_cos(subtopic , file_id , in_dir)
                
                print("topic_score:    ",topic_score)
                print("pm_score:    ",pm_score)
                file_value = lamc * topic_score + (1.0-lamc) * 10 * pm_score
            ...
            list_re_doc_id , list_unre_doc_id , dict_keyword , dict_subtopic_cnt , list_doc_untopic= interact_with_jig(jig, docs_result, in_dir, 1, dict_subtopic_cnt,list_doc_untopic)
