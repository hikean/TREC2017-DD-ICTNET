...
dict_id_key = json.load(codecs.open("data/id2key.json","r") )
...
def judge_if_untopic_doc(in_path,int_type):
...
def add_untopic_doc(in_path , file_id ,int_type):
    js = json.load(codecs.open(in_path, "r") )
    if int_type == 1:
        line = js["content_p"]
    else:
...
def interact_with_jig(jig, docs, in_dir , interact_times , dict_subtopic_cnt , list_doc_untopic, int_type):
    global int_untopic
    ...
    for i in range(interact_times):
        ...
        for _ in rslt:
            print ("rslt:", _)
            if int_type == 1:
                topic_id = str( dict_key_id[ _["doc_id"] ])
            else:
                topic_id = _["doc_id"]
            if _["on_topic"] == '0':
                #list_unre_doc_id.append( _["doc_id"] )
                
                add_untopic_doc( in_dir.format( topic_id ) , topic_id ,int_type)
                list_doc_untopic.append(  topic_id )
                int_untopic += 1
                continue
            ...
            if flag_choose == 1:
                list_doc_s.append( topic_id )
                list_re_doc_id.append( topic_id  )
                
            else:
                list_unre_doc_id.append( topic_id )
                pass
...
def read_json_file(in_path, id , int_type):
    global corp
...
def get_lda(out_dir,topic_name):
    ...
    file_ldaOut = codecs.open(out_dir + "{}-{}-{}-{}-{}-{}-{}-lda_topics-rocchio-nyt.txt".format(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5],sys.argv[6],topic_name), "w", "utf-8")
    file_ldaOut.write(str(ldaOut))
    
    file_id_lda = codecs.open(out_dir + "{}-{}-{}-{}-{}-{}-{}-fileid_lda-rocchio-nyt.txt".format(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5],sys.argv[6],topic_name), "w", "utf-8")
...
def get_dict_key_word(in_dir , doc_id , cnt_key_word , int_type):
...
def rocchio_get_new_query(str_query , list_re_doc_id , list_unre_doc_id , in_dir , dict_keywords):
    ...
    for doc_id in list_re_doc_id:
        dict_key_word = get_dict_key_word(in_dir, doc_id, 10 , int_type)
...
if __name__ == '__main__':
    ...
    global list_doc_q,list_doc_s,lama,lamb,corp,get_id,lsh,int_untopic
    """
    ...
    jig = JigClient("",tot_itr_times=tot_itr_times,base_jig_dir="/home/zhangwm/Software/dkjig/jig/trec-dd-jig/")
    ...
    for ebola_query in list_query[0:int(sys.argv[6])]:
        ...
        for tt in range( int(sys.argv[4]) ):
            ...
            if int_type == 1:
                int_n_doc = int(sys.argv[5]) + 100
            else:
                int_n_doc = int(sys.argv[5])
            docs = retrieval_top_k_doc_full(topic_query, solr, int_n_doc, query_range)
            ...
            for dict_doc in docs:
                doc_id = ""
                if ebola_query[0][0] == 'D':
                    doc_id = str(dict_key_id[ dict_doc[1] ] )
                else:
                    doc_id = dict_doc[1]
                
                if doc_id not in list_doc_s:
                    #list_doc_solr.append( dict_doc[1] )
                    if doc_id in list_doc_untopic:
                        continue
                    int_cnt_list_q += 1
                    if int_cnt_list_q <= 30:
                        list_doc_q.append( doc_id )
                    else:
                        list_much.append( dict_doc )
                    dict_fileid_score[ doc_id ] = float(dict_doc[2] )  
                    read_json_file( in_dir.format( doc_id ), doc_id ,int_type)
            
            #print("list_doc_s:    ",list_doc_s)
            ...
            in_dir_fileid_lda = "data/lda/{}-{}-{}-{}-{}-{}-{}-fileid_lda-rocchio-nyt.txt".format(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5],sys.argv[6],topic_name)
            ...
            for tuple in list_score:
                if judge_if_untopic_doc( in_dir.format(tuple[0]) , int_type ) == False:
                    if ebola_query[0][0] == 'D':
                        dict_doc_copy = (0 , dict_id_key[ tuple[0] ] , tuple[1] )
                    else:
            ...
            if int_choose < 5:
                for tuple_doc in list_much:
                    if int_type == 1:
                        doc_id = str(dict_key_id[ tuple_doc[1] ] )
                    else:
                        doc_id = tuple_doc[1]
                    if judge_if_untopic_doc( in_dir.format(doc_id) , int_type ) == False:
                        docs_result.append(tuple_doc)
                        int_choose += 1
                    else:
                        list_doc_untopic.append( doc_id )
                    if int_choose == 5:
                        break
                    
            print("docs_result: ",docs_result)
            
            list_re_doc_id , list_unre_doc_id , dict_keyword , dict_subtopic_cnt , list_doc_untopic = interact_with_jig(jig, docs_result, in_dir, 1, dict_subtopic_cnt,list_doc_untopic , int_type)
            
            if int_untopic >= 10 and (len(list_doc_s) > 0):
                break
