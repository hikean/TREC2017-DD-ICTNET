...
def get_lda(out_dir,topic_name):
    ...
    file_ldaOut = codecs.open(out_dir + "5-{}-{}-{}-{}-{}-{}-{}-lda_topics-rocchio-nyt.txt".format(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5],sys.argv[6],topic_name), "w", "utf-8")
    file_ldaOut.write(str(ldaOut))
    
    file_id_lda = codecs.open(out_dir + "5-{}-{}-{}-{}-{}-{}-{}-fileid_lda-rocchio-nyt.txt".format(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5],sys.argv[6],topic_name), "w", "utf-8")
...
def rocchio_get_new_query(str_query , list_re_doc_id , list_unre_doc_id , in_dir , dict_keywords):
    ...
    for doc_id in list_unre_doc_id:
        dict_key_word = get_dict_key_word(in_dir, doc_id, 3)
        for word in dict_key_word:
            if word not in dict_query:
                dict_query[word] = -dict_key_word[word]
            else:
...
if __name__ == '__main__':
    ...
    jig = JigClient("",tot_itr_times=tot_itr_times,base_jig_dir="/home/zhangwm/Software/dk_jig5/jig/trec-dd-jig/")
    ...
    for ebola_query in NYT_TOPICS[0:int(sys.argv[6])]:
        ...
        for tt in range( int(sys.argv[4]) ):
            ...
            in_dir_fileid_lda = "data/lda/5-{}-{}-{}-{}-{}-{}-{}-fileid_lda-rocchio-nyt.txt".format(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5],sys.argv[6],topic_name)
