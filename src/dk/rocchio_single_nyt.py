...
def get_lda(out_dir,topic_name):
    ...
    file_ldaOut = codecs.open(out_dir + "rocchio-single-{}-{}-{}-{}-{}-{}-lda_topics.txt".format(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5],topic_name), "w", "utf-8")
    file_ldaOut.write(str(ldaOut))
    
    file_id_lda = codecs.open(out_dir + "rocchio-single-{}-{}-{}-{}-{}-{}-fileid_lda.txt".format(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5],topic_name), "w", "utf-8")
...
def get_dict_key_word(in_dir , doc_id , cnt_key_word):
    ...
    list_word = jieba.cut( js["content"]["full_text"] , cut_all = False)
    cnt_word = {}
    cnt_word_in_doc = 0.0
    for word in list_word:
        if (word not in stopword) and (len(word) > 1):
...
def seven_str(str_docid):
    str_ans = str_docid
    for i in range(7-len(str_docid)):
        str_ans = "0" + str_ans
    return str_ans
...
if __name__ == '__main__':
    ...
    for ebola_query in NYT_TOPICS[0:int(sys.argv[6])]:
        ...
        for tt in range( int(sys.argv[4]) ):
            ...
            in_dir_fileid_lda = "data/lda/rocchio-single-{}-{}-{}-{}-{}-{}-fileid_lda.txt".format(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5],topic_name)
            ...
            for doc_id in docs_result_id:
                for dict_doc in docs:
                    if  dict_doc[1] == doc_id:
                        docs_result.append(dict_doc)
                        break
            
            #idf_dictionary = get_idf_dictionary(list_doc_solr, in_dir, int_cnt_doc_solr )
            
            list_re_doc_id , list_unre_doc_id , dict_keyword = interact_with_jig(jig, docs_result, 1)
