...
class LangModel(object):
    ...
    def __init__(self, model, ngram=(1,), miu=200.0):
        self.ngram, self.model = ngram, model
        self.msum, self.miu = sum(model.values()), miu
    ...
    def get_PwC(self, key):
        if isinstance(key, tuple):
            key = key[0]
        if key in "der die in werden eine ebola mit und":
            print self.model.get(key)
    ...
    def calcu_PQD(self, documentc, queryc):
    ...
    def compute_PwD(self, document, miu=None):
        if miu is None:
            miu = self.miu
        dsum = sum(document.values())
        return {key: (document.get(key, 0.0) + miu * self.get_PwC(key)) /
                (dsum + miu) for key in document}
    ...
    def get_candidate_words(self, documents, query, ngram=None):
        ...
        """
        query = self.cut_words(query)
        PQD = self.compute_PQD(documents, query, ngram)
        result = self.compute_PwR(documents, query, PQD, ngram)
